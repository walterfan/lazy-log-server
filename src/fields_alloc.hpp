...
namespace detail
{
    ...
    /** A non-thread-safe static memory pool implementation.
    
        This class manages a fixed-size memory block and provides
        allocation and deallocation functions for that block. It is
        designed to be used as a low-level memory allocator for
        higher-level data structures.
    
        The `construct` function creates a new instance of the
        `static_pool` class, allocating the necessary memory for the
        fixed-size block. The `share` function returns a reference to
        the existing instance, incrementing the reference count.
        The `destroy` function decrements the reference count and
        deallocates the memory if the reference count reaches zero.
    
        The `alloc` function allocates a block of memory from the
        fixed-size pool, throwing `std::bad_alloc` if the requested
        size exceeds the remaining space.

        The `dealloc` function decrements the allocation count,
        resetting the pool pointer to the beginning of the block
        when the last allocation is deallocated.
    */
    struct static_pool
    {
        std::size_t size_;
